...
public static class JwtTokenKeys
{
    ...
    public static JwtUser? GetJwtUser(this HttpContext context)
    {
        ...
        var medium = user.GetClaimValue(ProfileMedium).FirstOrDefault();

        var large = user.GetClaimValue(ProfileLarge).FirstOrDefault();
        ...
        return new JwtUser(
            ...
            TokenIssuedAt: long.Parse(user.GetClaimValue(Iat)[0], CultureInfo.InvariantCulture)
                .ToDateTime(),
            TokenExpiresAt: long.Parse(user.GetClaimValue(Exp)[0], CultureInfo.InvariantCulture)
                .ToDateTime(),
            ...
            ProfileMedium: Uri.TryCreate(medium, UriKind.Absolute, out var mediumUri)
                ? mediumUri
                : null,
    ...
    private static Collection<string> GetClaimValue(
