...
public static class IcsCalendarMapper
{
        ...
        CalendarLinkEntity calendarLinkEntity)
    {
        ...
        var events = calendar
            .Events
            .Select(e => e.ToDomain(calendarLinkEntity))
    ...
    private static string? ExtractCalendarTitle(Ical.Net.Calendar icsCalendar)
    {
        ...
        var calName = icsCalendar.Properties
            .FirstOrDefault(p => p.Name.Equals(CalendarNameHeader, StringComparison.OrdinalIgnoreCase))
            ?.Value?.ToString();
        ...
        var calDesc = icsCalendar.Properties
            .FirstOrDefault(p => p.Name.Equals(CalendarDescriptionHeader, StringComparison.OrdinalIgnoreCase))
            ?.Value?.ToString();
