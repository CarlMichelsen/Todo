...
{
    public async Task Handle(DispatchEventCommand command, CancellationToken cancellationToken)
    {
        ...
        var connectionSearchTasks = command.ServerEvent.Destination.Recipients.Select(destination =>
            connectionRegistry.GetByUserId(destination, cancellationToken)
        );

        var connectionSearchResult = await Task.WhenAll(connectionSearchTasks);
        var connections = connectionSearchResult.SelectMany(x => x);

        var eventDispatchTasks = connections.Select(connection =>
            connection.DispatchEvent(command.ServerEvent)
        );
    ...
    static partial void LogSentEventNameToRecipients(
        ILogger<DispatchEventCommandHandler> logger,
        string eventName,
        string recipients
    );
