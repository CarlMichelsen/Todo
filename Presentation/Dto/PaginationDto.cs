...
/// </summary>
public record PaginationDto<T>(IEnumerable<T> Data, int Page, int PageSize, int TotalCount)
    where T : class
{
    /// <summary>
    ...
    /// </summary>
    public PaginationDto<TResult> Map<TResult>(Func<T, TResult> mapper)
        where TResult : class => new(Data.Select(mapper), Page, PageSize, TotalCount);
}
